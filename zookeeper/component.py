...
class Component:
    ...
    def configure(self, conf, name=None, parent=None, interactive=False):
        ...
        for k, v in non_component_annotations:
            ...
            # instance, no action needs to be taken. Using `hasattr` is not
            # safe, as it is implemented with `getattr` wrapped in a try-catch
            # (Python is terrible), so we need to check `dir(self)`.
            elif k in dir(self):
                pass
